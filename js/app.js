...
function validateStep(step) {
  ...
  if (step === 2) {
    ...
    // Validate mobile digit count per country
    const nat = document.querySelector('[name="nationality"]')?.value;
    const digitCounts = { Uganda:[9], Kenya:[9,10], Philippines:[10], India:[10], Ethiopia:[9], Bangladesh:[10,11] };
    if (mobile && mobile.value && nat && digitCounts[nat]) {
      const len = mobile.value.replace(/\D/g,"").length;
      if (!digitCounts[nat].includes(len)) {
        mobile.classList.add("error");
        alert(`رقم الجوال يجب أن يكون ${digitCounts[nat].join(" أو ")} أرقام لـ ${nat}\nMobile must be ${digitCounts[nat].join(" or ")} digits for ${nat}`);
        valid = false;
      }
    }
    const salary = section.querySelector('[name="monthlySalary"]');
    if (salary && salary.value) {
      const s = parseInt(salary.value);
      if (s < 850 || s > 2000) {
        salary.classList.add("error");
        alert("الراتب يجب أن يكون بين 850 و 2000 ريال\nSalary must be between 850 and 2000 SAR");
        valid = false;
      }
    }
  }
  ...
  if (step === 3) {
    const pp = section.querySelector('[name="passportNumber"]');
    if (pp && pp.value) {
      const ppRegex = /^[A-Z][A-Z0-9]{6,8}$/;
      if (!ppRegex.test(pp.value.toUpperCase())) {
        pp.classList.add("error");
        alert("رقم الجواز يجب أن يبدأ بحرف كبير، 7-9 خانات أحرف وأرقام\nPassport must start with a letter, 7-9 characters (A-Z, 0-9)");
        valid = false;
      } else {
        pp.value = pp.value.toUpperCase(); // Auto capitalize
      }
    }
...
const response = await fetch(`${API_URL}/api/generate-cv`, {
  ...
  body: formData,
    ...
    a.href = url;
    a.download = `${data.fullName.replace(/\s+/g, "_")}_${data.passportNumber || "PP"}.pdf`;
